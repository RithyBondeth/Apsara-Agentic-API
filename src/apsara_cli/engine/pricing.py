...
def pricing_for_model(model: str) -> tuple[Optional[dict[str, float]], str]:
    ...
    if entry and entry.input_cost_per_million == 0 and entry.output_cost_per_million == 0:
        return {
            "input": 0.0, "output": 0.0, "cache_read": 0.0, "cache_write": 0.0,
    ...
    return {
        ...
        "cache_read": float(info.get("cache_read_input_token_cost", input_cost)),
        "cache_write": float(info.get("cache_creation_input_token_cost", input_cost)),
    }, source
